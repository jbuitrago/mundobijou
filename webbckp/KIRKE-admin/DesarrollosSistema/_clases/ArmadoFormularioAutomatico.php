...
class ArmadoFormularioAutomatico extends PlantillaReemplazos {
    ...
    private $_componentes_habilitados = array('Texto', 'TextoLargo', 'Desplegable');
    ...
    function __construct($id_tabla) {
        ...
        $this->_idioma = Generales_Idioma::obtener();
    }
    ...
    public function obtenerFormulario() {
        ...
        if ($this->_captcha === true) {
            ...
            $armado_tabla .= '
                ...
                <td class="frm_td_captcha">
                ' . $captcha_img . '
                </td>
                <td class="frm_td_captcha_input">
    ...
    private function _armarArray() {
        ...
        if (count($this->m_mail_destinatario) > 0) {
            $frm_automatico->mailDestinatario($this->m_mail_destinatario[$num]['mail'], $this->m_mail_destinatario[$num]['nombre']);
        }
        ...
        if ($this->m_mail_respuesta != '') {
            $frm_automatico->mailRespuesta($mail);
        }

        if (count($this->m_asunto) > 0) {
            $frm_automatico->asunto($this->m_asunto[$num]['asunto'], $this->m_asunto[$num]['asunto_matriz_reemplazo']);
        }
    ...
    private function _002_CampoTextoLargo($cp) {
        ...
        $titulo = $cp['idioma_' . $this->_idioma];
        $elemento = '<textarea type="text" name="' . $nombre . '" id="' . $nombre . '" class="frm_textarea" ' . $maxlength . $obligatorio . ' etiqueta="' . $titulo . '" rows="' . $cp['alto'] . '" style="width:' . $cp['ancho'] . 'px;"></textarea>' . $error . $filtro;
