...
class Componentes_color_ComponenteInsercion {
    ...
    private function _insertarConfiguracion() {
        ...
        if ($tabla_tipo == 'registros') {
            ...
            $componente_carga->crearModificarColumna($tabla_nombre, $this->_tbCampo, $tb_campo_anterior, $this->_pv['tipo_dato'], $this->_pv['largo'], $es_nulo);
        ...
        if ($_POST['color_inicial'] != $this->_pv['color_inicial']) {
            $componente_carga->consultaParametro($id_componente, 'color_inicial', Bases_InyeccionSql::consulta($_POST['color_inicial']));
        } elseif ($_GET['accion'] == 'ComponenteModificacionInsercion') {
            $componente_carga->consultaParametroEliminar($id_componente, 'color_inicial');
        }
