...
class Componentes_OpcionesPredefinidas_RegistroExportar extends Armado_Plantilla {
    ...
    public function get() {
        if(isset($this->_opciones[$this->_valor])){
            $this->_valor = $this->_opciones[$this->_valor];
        }else{
            $this->_valor = '-';
        }
    ...
    private function _sqlEstructura() {
        ...
        $titulo[0] = '  `' . $this->_dcp['tb_campo'] . '` varchar(3) ' . $obligatorio . ',' . "\n";
