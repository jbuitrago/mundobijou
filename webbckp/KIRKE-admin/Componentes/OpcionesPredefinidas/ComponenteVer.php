...
class Componentes_OpcionesPredefinidas_ComponenteVer extends Armado_Plantilla {
...
// metodos de acciones
    ...
    private function _armado() {
        ...
        for ($i = 1; $i <= 15; $i++) {
            $plantilla['valor_' . $i] = $this->_dcp['valor_' . $i];
            ...
            if (is_array(Inicio::confVars('idiomas'))) {
                foreach (Inicio::confVars('idiomas') as $key => $value) {
                    ...
                    if (isset($this->_dcp['etiqueta_' . $i . '_' . $idioma_s_ext])) {
                        $etiqueta_idioma[$idioma_s_ext] = $this->_dcp['etiqueta_' . $i . '_' . $idioma_s_ext];
                    } else {
                        $etiqueta_idioma[$idioma_s_ext] = '';
                    }
                }
                $plantilla['etiqueta_valor_' . $i] = Armado_EtiquetaIdiomas::armar('etiqueta_' . $i, $etiqueta_idioma);
            }
        }
