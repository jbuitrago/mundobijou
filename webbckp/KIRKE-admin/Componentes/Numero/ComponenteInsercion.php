...
class Componentes_Numero_ComponenteInsercion {
    ...
    private function _insertarConfiguracion() {
        ...
        $largo = Bases_InyeccionSql::consulta($_POST['largo']);
        $decimales = Bases_InyeccionSql::consulta($_POST['decimales']);

        if ($decimales != 0) {
            $formato_dato = $largo . ',' . $decimales;
        } else {
            $this->_pv['tipo_dato'] = 'numero';
            $formato_dato = $largo;
        }
        ...
        if ($tb_tipo == 'registros') {
            ...
            $componente_carga->crearModificarColumna($tb_nombre, $this->_tbCampo, $tb_campo_anterior, $this->_pv['tipo_dato'], $formato_dato, $es_nulo);
        ...
        if ($decimales != $this->_pv['decimales']) {
            $componente_carga->consultaParametro($id_componente, 'decimales', $decimales);
        } elseif ($_GET['accion'] == 'ComponenteModificacionInsercion') {
            $componente_carga->consultaParametroEliminar($id_componente, 'decimales');
        }
