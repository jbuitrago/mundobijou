...
class Componentes_DesplegableMultiple_ComponenteInsercion {
    ...
    private $_seleccionar_alta; // permite la seleccion o no de un grupo diferente al filtrado
    private $_pv;
        ...
        if ($_GET['accion'] == 'ComponenteAltaInsercion') {
            ...
            for ($i = 0; $i <= 9; $i++) {
                if (isset($_POST['desplegable_' . $i]) && ($_POST['desplegable_' . $i] != '')) {
                    if ($i == 1)
                        $coma = ',';
                    $valore_relacionados .= $coma . $_POST['desplegable_' . $i];
                }
            }
