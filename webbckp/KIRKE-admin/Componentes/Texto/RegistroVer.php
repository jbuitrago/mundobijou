...
class Componentes_Texto_RegistroVer extends Armado_Plantilla {
    ...
    private function _registroVer() {
        ...
        if (!isset($this->_dcp['ocultar_vista']) || ($this->_dcp['ocultar_vista'] == 'n')) {
            ...
            $mostrar = $this->_codigoHtml($this->_dcp['permite_html'], Generales_Post::obtener($this->_valor));
            $mostrar = $this->_verLinksTexto($mostrar);
...
// metodos especiales
    ...
    private function _verLinksTexto($texto) {
        ...
        if (isset($this->_dcp['link_mail']) && ($this->_dcp['link_mail'] == 's')) {
            $texto = preg_replace_callback("/[a-zA-Z0-9_\-\.]+@[a-zA-Z0-9\_\-\.]+\.[a-zA-Z0-9\_\-\.]+/", array('Componentes_Texto_RegistroVer', '_verLinksTextoMail'), $texto);
        }

        if (isset($this->_dcp['link_url']) && ($this->_dcp['link_url'] == 's')) {
            $texto = preg_replace_callback("/\b(?:(?:https?|ftp):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]/i", array('Componentes_Texto_RegistroVer', '_verLinksTextoURL'), $texto);
        }
