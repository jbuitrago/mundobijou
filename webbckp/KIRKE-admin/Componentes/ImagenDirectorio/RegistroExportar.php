...
class Componentes_ImagenDirectorio_RegistroExportar extends Armado_Plantilla {
    ...
    public function get() {
        if ($this->_valor != '') {
            ...
            $this->_valor = preg_replace("/^([0-9]+)[_]([0-9]+)[_]/", '', $this->_quitarSubdirectorio($this->_valor));
        }else{
            $this->_valor = '-';
        }
    ...
    private function _pdf() {
        if($this->_imagen!=''){
            $valor['imagen'] =  $this->_directorio.'/'.$this->_imagen;
        }
    ...
    private function _quitarSubdirectorio($nombre) {
        $nombre_array = explode('/', $nombre);
        if(count($nombre_array)>1){
            return $nombre_array[1];
        }
        return $nombre;
    }
