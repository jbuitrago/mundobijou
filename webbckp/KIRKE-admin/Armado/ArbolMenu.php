...
class Armado_ArbolMenu {

    static function get() {
        return self::_armarMenu(Consultas_Menu::RegistroConsultaTodosCantidadLinks(__FILE__, __LINE__, true));
    }

    static private function _armarMenu($menus) {
        $menu_armar = '';
        $nivel_ant = array('', '', '', '');
        if (is_array($menus)) {
            foreach ($menus as $id => $linea) {
                $nivel_act = array($linea['nivel1_orden'], $linea['nivel2_orden'], $linea['nivel3_orden'], $linea['nivel4_orden']);
                $nivel = Generales_MenuObtenerNivel::nivel($linea['nivel1_orden'], $linea['nivel2_orden'], $linea['nivel3_orden'], $linea['nivel4_orden']);
                ...
                $menu_armar .= self::_nivelIr($nivel_ant, $nivel_act, $linea['id_menu'], $linea['menu_nombre'], $linea['cantidad'], $nivel, $linea_siguiente);
                $nivel_ant = $nivel_act;
            }
            // cierre del menu
    ...
    static private function _nivelIr($nivel_ant, $nivel_act, $id_menu, $menu_nombre, $cantidad, $nivel, $linea_siguiente) {
        $subir = '';
        $bajar = '';
        for ($i = 0; $i < 4; ++$i) {
            if (($nivel_ant[$i] != '') && ($nivel_act[$i] == '')) {
                $bajar .= '</ul>' . "\n";
            }
        ...
        if ($nivel_act[$nivel - 1] > 1) {
            $parametros = array('kk_generar' => '0', 'accion' => '15', 'menu_subir' => $id_menu);
        ...
        if ($linea_siguiente !== false) {
            if (
                ...
                $parametros = array('kk_generar' => '0', 'accion' => '15', 'menu_bajar' => $id_menu);
                $link = './index.php?' . Generales_VariablesGet::armar($parametros, 's');
        ...
        $subir .= '<li id="' . $id_menu . '"  class="folder" data="cantidad: \'' . $cantidad . '\', nivel: \'' . $nivel . '\'"><span><span><span style="background-color: #F0F0F0;">' . $menu_subir . $menu_bajar . '</span>&nbsp;' . $menu_nombre . '</span> <span class="tree_cantidad_gris">[' . $cantidad . ']</span></span>' . "\n";
        return $bajar . $subir;
    }
