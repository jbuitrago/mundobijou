...
class ArmadoFormulario extends PlantillaReemplazos {
    ...
    public static $ocultar_errores = array();
    ...
    public function campoValor($tipo, $parametro1 = null, $parametro2 = null) {
        switch ($tipo) {
            case 'mail':
                ...
                $this->_formulario[$this->_kk_control_form][$this->_nombreCampo]['mail'] = 's';

                self::$_formulario_tpl[$nombreArchivo][$this->_nombreCampo]['valor'] = $tipo;
                self::$_formulario_tpl[$nombreArchivo][$this->_nombreCampo]['mail'] = 's';

                break;
            case 'filtro':
                ArmadoEncabezados::encabezadosEstaticos("<script type=\"text/javascript\"> $(document).ready(function(){ solo_texto_permitido('" . $this->_nombreCampo . "','" . $parametro1 . "') }); </script>");
                break;
        }
    }
    ...
    public function asunto($asunto, $asunto_matriz_reemplazo = null) {
        ...
        $this->m_mail_asunto[$num]['asunto'] = $asunto;
        $this->m_mail_asunto[$num]['asunto_matriz_reemplazo'] = $asunto_matriz_reemplazo;
    }
    ...
    // oculta los errores de los campos en el template
    public function ocultarErrores() {
        self::$ocultar_errores[$this->_kk_control_form] = true;
    }
            ...
            if (count($this->_formulario) > 0) {
                foreach ($this->_formulario[$this->_kk_control_form] as $key => $valor) {
                    if ($valor['obligatorio'] == 's') {
                        ...
                        } elseif (isset($_FILES[$key]) && ($_FILES[$key]['name'] == '')) {// && ($_FILES[$key]['name'] == '')) {
                            $validacion = false;
                        } elseif (!isset($_FILES[$key]) && (!isset($_POST[$key]) || (isset($_POST[$key]) && ($_POST[$key] == '')))) {
                            $validacion = false;
                        }
                    }
                    if (isset($valor['mail']) && ($valor['mail'] == 's') && isset($_POST[$key]) && ($_POST[$key] != '') && !filter_var($_POST[$key], FILTER_VALIDATE_EMAIL)) {
                        $validacion = false;
                    }
                }
