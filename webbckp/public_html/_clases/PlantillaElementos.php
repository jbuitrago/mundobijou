...
class PlantillaElementos extends PlantillaReemplazos {
    ...
    static public function plantilla($plantilla, $nombreArchivo) {
        ...
        // Quitar php embebido
        $plantilla = str_replace(array('<?', '<?php', '?>', '{$$_'), '', $plantilla);

        // Encabezados antiguos ============================================
        
        $plantilla = str_replace('{$#kk_encabezados}', '{headers}', $plantilla);
        ...
        // HTML reemplazo etiqueta error peronalizado =====================
    ...
    static private function _htmlReemplazar($coincidencias) {
        ...
        if (preg_match('/placeholder=\"([a-zA-Z0-9._.&.#.%.\-.;.:.\s\*]+)\"/', $coincidencias[2], $coincidencia)) {
            $placeholder = $coincidencia[1];
        } else {
            $placeholder = '';
        }

        if (preg_match('/autocomplete=\"off\"/', $coincidencias[2], $coincidencia)) {
            $autocomplete = $coincidencia[1];
        } else {
            $autocomplete = '';
        }

        if (strpos($coincidencias[2], ' readonly="readonly"') === false) {
            $readonly = false;
        } else {
            $readonly = true;
        }
        ...
        if ($tipo == "captcha_img") {
            return "<?=\$this->_htmlCaptcha('" . self::$_formulario . "', '" . $class . "', '" . $estilo . "')?>";
        }
        ...
        return self::_htmlArmado($tipo, $nombre, $opciones, $seleccionado, $class, $estilo, $html, $width, $rows, $nombre_compuesto, $placeholder, $autocomplete, $readonly);
    }
    ...
    static private function _linksTemplate($n) {
        ...
        return "<?=self::_links( " . $array_links . " )?>";
    }
    ...
    static private function _htmlArmado($tipo, $nombre, $opciones, $seleccionado, $class, $estilo, $html, $width = null, $rows = null, $nombre_compuesto, $placeholder, $autocomplete, $readonly) {
        ...
        if (
                ($nombre_control === false) && (
                (ArmadoFormulario::getCampo(self::$_nombreArchivo, $nombre, 'obligatorio') == 's') ||
                (ArmadoFormulario::getCampo(self::$_nombreArchivo, $nombre, 'mail') == 's')
                )
        ) {
            if (!isset(ArmadoFormulario::$ocultar_errores[self::$_formulario])) {
                $error = '<div id="VC_' . $nombre . '" class="VC_error"></div>';
            } else {
                $error = '';
            }
        } elseif (($tipo != 'captcha') && ($nombre_control === true)) {
            $error = '<?php if(ArmadoFormulario::getCampo("' . self::$_nombreArchivo . '", ' . $nombre_compuesto . ', "obligatorio")=="s"){ ?><div id="VC_' . $nombre . '" class="VC_error"></div><?php } ?>';
        } elseif ($tipo == 'captcha') {
            if (!isset(ArmadoFormulario::$ocultar_errores[self::$_formulario])) {
                $error = '<div id="VC_captcha" class="VC_error"></div>';
            } else {
                $error = '';
            }
        }
        ...
        switch ($tipo) {
            case 'input':
                ...
                return '<input type="text" name="' . $nombre . '" id="' . $nombre . '" value="' . $seleccionado . '"' . $class . $estilo . $maxlength . $obligatorio . $valor . $accion . $etiqueta_campo . $placeholder . $readonly . ' >' . $error;
                break;
            case 'textarea':
                if (ArmadoFormulario::getCampo(self::$_nombreArchivo, $nombre, 'obligatorio') == 's') {
                    $obligatorio = ' tipo="obligatorio" ';
                ...
                return '<textarea name="' . $nombre . '" id="' . $nombre . '"' . $rows . $class . $estilo . $maxlength . $obligatorio . $etiqueta_campo . $placeholder . '>' . $seleccionado . $readonly . '</textarea>' . $error;
                break;
            case 'hidden':
                if (ArmadoFormulario::getCampo(self::$_nombreArchivo, $nombre, 'obligatorio') == 's') {
                    $obligatorio = ' tipo="obligatorio" ';
            ...
            case 'password':
                ...
                return '<input type="password" name="' . $nombre . '" id="' . $nombre . '" value="' . $seleccionado . '"' . $class . $estilo . $maxlength . $obligatorio . $etiqueta_campo . $placeholder . $readonly . '>' . $error;
                break;
            case 'file':
                if (ArmadoFormulario::getCampo(self::$_nombreArchivo, $nombre, 'obligatorio') == 's') {
                    $obligatorio = '<input type="hidden" name="kk-file-' . $nombre . '" id="kk-file-' . $nombre . '" valor="' . $nombre . '">';
                } elseif ($nombre_control === true) {
                    $obligatorio = '<?php if(ArmadoFormulario::getCampo("' . self::$_nombreArchivo . '", ' . $nombre_compuesto . ', "obligatorio")=="s"){ ?> tipo="obligatorio"<?php } ?>';
                }
                return '<input type="file" name="' . $nombre . '" id="' . $nombre . '"' . $class . $estilo . $etiqueta_campo . $readonly . '>' . $obligatorio . $error;
                break;
            case 'select':
                if (ArmadoFormulario::getCampo(self::$_nombreArchivo, $nombre, 'obligatorio') == 's') {
                    $obligatorio = ' tipo="obligatorio" ';
                ...
                $armado = '<select name="' . $nombre . '" id="' . $nombre . '"' . $class . $estilo . $obligatorio . $etiqueta_campo . $readonly . ' >';
                $armado .= "<?=\$this->_htmlArmadoSelect( " . $opciones . ", " . $seleccionado . " )?>";
                $armado .= '</select>';
                return $armado . $error;
                break;
            case 'checkboxes':
                $armado = "<?=\$this->_htmlArmadoCheckboxes( '" . $nombre . "', " . $opciones . ", " . $seleccionado . ", '" . $estilo . "', '" . $class . "', '" . $html . "', '" . $nombre_compuesto . "', '" . self::$_nombreArchivo . "' )?>";
                return $armado;
                break;
            case 'radios':
                $armado = "<?=\$this->_htmlArmadoRadios( '" . $nombre . "', " . $opciones . ", " . $seleccionado . ", '" . $estilo . "', '" . $class . "', '" . $html . "', '" . $nombre_compuesto . "', '" . self::$_nombreArchivo . "' )?>";
                return $armado;
                break;
            case 'captcha':
                $nombre = 'captcha';
                ...
                $armado = '<input type="text" name="' . $nombre . '" id="' . $nombre . '" value="' . $seleccionado . '"' . $class . $estilo . $maxlength . $obligatorio . $valor . $accion . $etiqueta_campo . $placeholder . $readonly . '>';
                return $armado . $error;
                break;
        }
    }
