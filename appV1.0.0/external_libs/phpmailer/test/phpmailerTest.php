...
class PHPMailerTest extends PHPUnit_Framework_TestCase {
    ...
    public function testCustomValidator() {
        ...
        $this->assertFalse(
                PHPMailer::validateAddress(
                        'userexample.com', function ($address) {
                    return (strpos($address, '@') !== false);
                }
        ...
        $this->assertTrue(
                $this->Mail->addAddress('user@example.com'), 'Custom default validator false negative'
        );
        $this->assertFalse(
                //Need to pick a failing value which would pass all other validators
                //to be sure we're using our custom one
                $this->Mail->addAddress('bananas@example.com'), 'Custom default validator false positive'
        );
        //Set default validator to PHP built-in
        PHPMailer::$validator = 'php';
        $this->assertFalse(
                //This is a valid address that FILTER_VALIDATE_EMAIL thinks is invalid
                $this->Mail->addAddress('first.last@example.123'), 'PHP validator not behaving as expected'
        );
    }
    ...
    public function testWordWrapMultibyte() {
        ...
        $my_body = str_repeat(
                ...
                '飛兒樂 團光茫飛兒樂 團飛兒樂 團光茫光茫飛兒樂 團光茫. ', 10
        );
        $nBodyLen = strlen($my_body);
        $my_body .= "\n\nThis is the above body length: " . $nBodyLen;
    ...
    public function testQuotedPrintable() {
        ...
        $t = str_replace("\n", "\r\n", $t);
        $this->assertEquals(
                $t, quoted_printable_decode($this->Mail->encodeQP($t)), 'Quoted-Printable encoding round-trip failed (Windows line breaks)'
        );
    }
...
EOT;
    ...
    public function testHtmlIso8859() {
        ...
        $this->Mail->msgHTML(
                mb_convert_encoding(
                        $content, "ISO-8859-1", mb_detect_encoding($content, "UTF-8, ISO-8859-1, ISO-8859-15", true)
                ), realpath(self::INCLUDE_DIR . 'examples')
        );
        $this->buildBody();
        $this->assertTrue(
                strpos($this->Mail->Body, $check) !== false, 'ISO message body does not contain expected text'
        );
        $this->assertTrue($this->Mail->send(), $this->Mail->ErrorInfo);
    }
...
EOT;
        $this->Mail->addEmbeddedImage(
                realpath(self::INCLUDE_DIR . 'examples/images/phpmailer.png'), 'my-attach', 'phpmailer.png', 'base64', 'image/png'
        );
        $this->buildBody();
        $this->assertTrue($this->Mail->send(), $this->Mail->ErrorInfo);
    }
...
EOT;
    ...
    public function testHTMLStringEmbedNoName() {
        ...
        if (!$this->Mail->addStringEmbeddedImage(
                        file_get_contents(realpath(self::INCLUDE_DIR . 'examples/images/phpmailer_mini.png')), md5('phpmailer_mini.png') . '@phpmailer.0', '', //intentionally empty name
                        'base64', 'image/png', 'inline')
        ) {
            $this->assertTrue(false, $this->Mail->ErrorInfo);
            return;
        }
    ...
    public function testLongBody() {
        ...
        $this->assertTrue(
                PHPMailer::hasLineLongerThanMax($this->Mail->Body), 'Test content does not contain long lines!'
        );
        $this->Mail->isHTML();
        $this->buildBody();
        ...
        $this->assertContains(
                'Content-Transfer-Encoding: quoted-printable', $message, 'Long line did not cause transfer encoding switch.'
        );
    }
    ...
    public function testShortBody() {
        ...
        $this->assertFalse(
                PHPMailer::hasLineLongerThanMax($this->Mail->Body), 'Test content contains long lines!'
        );
        $this->buildBody();
        $this->Mail->Encoding = '8bit';
        $this->Mail->preSend();
        ...
        $this->assertFalse(PHPMailer::hasLineLongerThanMax($message), 'Long line not corrected.');
        $this->assertNotContains(
                'Content-Transfer-Encoding: quoted-printable', $message, 'Short line caused transfer encoding switch.'
        );
    }
    ...
    public function testAddressSplitting() {
        ...
        $this->assertCount(
                2, $this->Mail->parseAddresses(
                        'Joe User <joe@example.com>, Jill User <jill@example.net>', false
                ), 'Failed to recognise address list'
        );
        //Test single address
        $this->assertNotEmpty(
                $this->Mail->parseAddresses(
                        'Joe User <joe@example.com>', false
        ...
        $this->assertNotEmpty(
                $this->Mail->parseAddresses(
                        'Tim "The Book" O\'Reilly <foo@example.com>', false
    ...
    public function testMIMEStructure() {
        ...
        $this->assertRegExp(
                ...
                "This is a multi-part message in MIME format./", $this->Mail->getSentMIMEMessage(), 'MIME structure broken'
        );
    }
    ...
    public function testSigningWithCA() {
        ...
        $this->Mail->sign(
                $certfile, $keyfile, $password, $cacertfile
        );
    ...
    public function testLineLength() {
        ...
        $this->assertTrue(
                PHPMailer::hasLineLongerThanMax($oklen . $badlen . $oklen), 'Long line not detected (middle)'
        );
    ...
    public function testConfirmReadingTo() {
        ...
        $this->assertTrue($this->Mail->send(), $this->Mail->ErrorInfo);
        $this->assertEquals(
                'test@example.com', $this->Mail->ConfirmReadingTo, 'Unexpected read receipt address');

        $this->Mail->ConfirmReadingTo = 'test@françois.ch';  //Address with IDN
        if ($this->Mail->idnSupported()) {
            $this->assertTrue($this->Mail->send(), $this->Mail->ErrorInfo);
            $this->assertEquals(
                    'test@xn--franois-xxa.ch', $this->Mail->ConfirmReadingTo, 'IDN address not converted to punycode');
        } else {
            $this->assertFalse($this->Mail->send(), $this->Mail->ErrorInfo);
        }
    }
    ...
    public function testConvertEncoding() {
        ...
        $this->assertEquals(
                array(array('test' . $domain, '')), $this->Mail->getToAddresses(), 'Bad "to" recipients');
        $this->assertEquals(
                array(array('test+cc' . $domain, '')), $this->Mail->getCcAddresses(), 'Bad "cc" recipients');
        $this->assertEmpty($this->Mail->getBccAddresses(), 'Bad "bcc" recipients');
        $this->assertEquals(
                array('test+replyto' . $domain => array('test+replyto' . $domain, '')), $this->Mail->getReplyToAddresses(), 'Bad "reply-to" addresses');
    }
    ...
    public function testDuplicateIDNRemoved() {
        ...
        $this->assertEquals(
                1, count($this->Mail->getToAddresses()), 'Bad count of "to" recipients');
        $this->assertEquals(
                1, count($this->Mail->getReplyToAddresses()), 'Bad count of "reply-to" addresses');
    }
