    ...
    <div id="content">
                            ...
                            <div class="col-sm-10 col-sm-offset-1">
                                ...
                                $subcategoria_actual = 0;
                                    ...
                                    foreach ($categorias as $c) {
                                        if ($c->id == $categoria) {
                                            $categoria_actual = $c->nombre.'-'.$c->id;
                                            echo '<h1>' . $c->nombre . '</h1>';
                                        }
                                    }
                                    ...
                                    foreach ($subcategorias as $s) {
                                        if ($s->id == $subcategoria) {
                                            $subcategoria_actual = $s->nombre.'-'.$s->id;
                                    ...
                                    <form class="form-inline">
                                        <div class="col-md-6 col-sm-6">
                                            <div class="products-number"> <strong>Mostrar</strong> 
                                                <a href="<?= base_url(); ?><?= $categoria_actual ?>/<?= $subcategoria_actual ?>/<?= (($sort == 'precio_mayorista')and ( $order == 'desc')) ? $sort . '_' . $order : $sort ?>/12/<?= 1 ?>" class="btn btn-default btn-sm <?= ($per_page == 12) ? 'btn-primary' : '' ?>">12</a> 
                                                <a href="<?= base_url(); ?><?= $categoria_actual ?>/<?= $subcategoria_actual ?>/<?= (($sort == 'precio_mayorista')and ( $order == 'desc')) ? $sort . '_' . $order : $sort ?>/24/<?= 1 ?>" class="btn btn-default btn-sm <?= ($per_page == 24) ? 'btn-primary' : '' ?>">24</a> 
                                                <a href="<?= base_url(); ?><?= $categoria_actual ?>/<?= $subcategoria_actual ?>/<?= (($sort == 'precio_mayorista')and ( $order == 'desc')) ? $sort . '_' . $order : $sort ?>/99999999/<?= 1 ?>" class="btn btn-default btn-sm <?= ($per_page == 99999999) ? 'btn-primary' : '' ?>">Todos</a> 
                                            </div>
                                        </div>
                                        <div class="col-md-6 col-sm-6">
                            ...
                            <?php if ($indice == 0): ?><div class="row products"><?php endif; ?>
                                ...
                                <div class="col-md-3 col-sm-4">
                                    ...
                                    <div class="product" id="producto_<?= $producto->id ?>">
                                        <div class="image" style="height: 180px;"> <a href="<?php echo site_url(strtolower($producto->categoria) . '/' . $producto->titulo . '-' . $producto->articulo . '-' . $producto->id); ?>"> <img src="<?= base_url() . 'uploads/' . $producto->file1; ?>" alt="<?= $producto->titulo ?> art.<?= $producto->articulo ?>" class="img-responsive image1"> </a>
                                            <div class="quick-view-button"> <a href="#" my-link="<?php echo site_url('web/modal_detalle/' .strtolower($producto->categoria) . '/' . $producto->titulo . '-' . $producto->articulo . '-' . $producto->id); ?>" data-toggle="modal" data-target="#product-quick-view-modal" class="btn btn-default btn-sm">Vista rápida</a> </div>
                                        </div>
                                        <!-- /.image -->
                                        <div class="text">
                                            <h3><a href="<?php echo site_url(strtolower($producto->categoria) . '/' . $producto->titulo . '-' . $producto->articulo . '-' . $producto->id); ?>"><?= $producto->titulo ?> art.<?= $producto->articulo ?></a></h3>
                                            ...
                                            <p class="text-center">
                                                <a href="<?php echo site_url(strtolower($producto->categoria) . '/' . $producto->titulo . '-' . $producto->articulo . '-' . $producto->id); ?>" class="btn btn-primary"><i class="fa fa-shopping-cart"></i> Comprar</a>
                                                <br><br>

                                                <a href="<?php echo site_url(strtolower($producto->categoria) . '/' . $producto->titulo . '-' . $producto->articulo . '-' . $producto->id); ?>"><i class="fa fa-external-link-square" aria-hidden="true"></i> VER DETALLE</a></p>
                                        </div>
                                        <!-- /.text -->
                                    </div>
        ...
        <script>
            function ordenar(order) {
                ...
                document.location = "<?= base_url(); ?><?= $categoria_actual ?>/<?= $subcategoria_actual ?>/" + order;
            }
