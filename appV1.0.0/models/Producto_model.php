...
class producto_model extends Default_model {
    ...
    function get_productos($limit, $start, $sort, $order, $categoria, $subcategoria, $filtro = '') {
    ...
    public function get_categoria_for_uri($producto_id) {

        $query = $this->db->query("SELECT categorias.nombre, categorias.id FROM producto "
                . "                INNER JOIN categorias ON(producto.categoria_id=categorias.id)"
                . "                 WHERE producto.id=$producto_id");

        if ($query->num_rows() > 0) {
            $row = $query->row();

            return (str_replace(' ', '-', $row->nombre)) . '-' . $row->id;
        }

        return '';
    }

    public function get_subcategoria_for_uri($producto_id) {

        $query = $this->db->query("SELECT subcategorias.nombre, subcategorias.id FROM producto "
                . "                INNER JOIN subcategorias ON(producto.sub_categoria_id=subcategorias.id)"
                . "                 WHERE producto.id=$producto_id");
