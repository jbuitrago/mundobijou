...
class Web extends MY_Controller {
    ...
    var $system = array("email_desarrollador" => "info@mtkstudio.com.ar", "email_administrador" => "consultas@mtkstudio.com");
    var $model = 'web_model';
    ...
    public function mail_registro($datos)
    {
        require_once APPPATH . '/external_libs/phpmailer/PHPMailerAutoload.php';
            ...
            //$mail->SMTPDebug  = 3;
            $mail->Host = "mail.c0770195.ferozo.com"; // SMTP a utilizar. Por ej. smtp.elserver.com
    ...
    public function checkout5() {
        ...
        if ($this->session->userdata('cliente_logged_in') && !empty($this->cart->contents()) && !empty($this->session->userdata('direccion_pedido')) && !empty($this->session->userdata('formapago_pedido'))) {
            ...
            if ($pedido_grabado['status']) {
                ...
                $data_index['pedido'] = $this->pedido->get_by_id($pedido_grabado['id']);
                ...
                $this->enviar_email(array('subject' => 'Pedido realizado con exito', 'pedido' => $data_index['pedido'], 'items' => $data_index['pedido_items'], 'email_to' => $this->cliente_logueado, 'extra' => $d), 'pedido');

                $this->enviar_email(array('subject' => 'Hay productos sin stock', 'items' => $data_index['pedido_items'], 'email_to' => 'negocifacundo@hotmail.com'), 'quiebre');
 ...
 public function cambiar_pass() {
    ...
    public function cart_add() {
        ...
        $this->form_validation->set_rules('quantity', 'Cantidad', 'required|xss_clean|trim');
    ...
    private function get_config_pagination($categoria, $sub_categoria, $page = 'productos') {
        ...
        $config["per_page"] = (!empty($this->uri->segment(5))) ? $this->uri->segment(5) : '2';
    ...
    private function contruct_uri() {
        ...
        $per_page = (!empty($this->uri->segment(5))) ? $this->uri->segment(5) : '2';
    ...
    public function send() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            //Estas dos líneas, cumplirían la función de encabezado (En mail() usado de esta forma: “From: Nombre <correo@dominio.com>”) de //correo.
            $mail->AddAddress($data['email']); // Esta es la dirección a donde enviamos
            $mail->IsHTML(true); // El correo se envía como HTML
