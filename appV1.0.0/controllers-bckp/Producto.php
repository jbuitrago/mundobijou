                ...
                if ($titulo != '') {
                    ...
                    if ($cant_cat != 0) 
                    {
                        ...
                        else
                        {
                            ...
                            if ($cant_sub != 0) {$id_subcat = $id_subcategoria->row();$subcategoria = $id_subcat->id;$ok = true;}
                            else{$ok = false;}
                        }
                        if ($ok) 
                        {
                            $oferta = $objWorksheet->getCellByColumnAndRow(5, $i)->getValue();
                            if (strlen($oferta) == 0){$oferta = 'NO';} 
                            else {$oferta = strtoupper($oferta);}
                            $nuevo = $objWorksheet->getCellByColumnAndRow(6, $i)->getValue();
                            if (strlen($nuevo) == 0) {$nuevo = 'NO';} else {$nuevo = strtoupper($nuevo);}
                            ...
                            if (($insert)and ( strlen($talles) != 0)) {
                                $array_talles = explode(",", ($talles . ",")); $data_talles = array();
                                foreach ($array_talles as $value1) { if (strlen($value1) != 0) { $id_talles = $this->db->query("SELECT * FROM talles WHERE UPPER(nombre) = UPPER('" . trim($value1) . "') AND borrado='no'"); if ($id_talles->num_rows() != 0) { $id_t = $id_talles->row(); $data_talles[] = array('producto_id' => $insert, 'talle_id' => $id_t->id); } else { $this->db->trans_rollback();echo '<link href="' . base_url() . 'assets/web/css/bootstrap.min.css" rel="stylesheet"><body><div class="modal-backdrop fade in" style="z-index:-1;"></div><div class="modalx fade"  role="dialog" style="display:block;opacity:1;"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><h3 class="modal-title"> Error al importar desde Excel</h3></div><div class="modal-body form" style="margin-bottom:15px;"><label class="control-label col-md-3" style="width:100%;">El talle ' . $value1 . ' de la celda ' . $i . ' no existe!</label></div><div class="modal-footer"><button type="button" onClick="history.back()" class="btn btn-danger" data-dismiss="modal">Atras</button></div></div></div></div></div></body>';die;  } } }
                                $this->obj_model->insert_multiple('producto_talles', $data_talles);
                            }

                            if (($insert)and ( strlen($colores) != 0)) {
                                $array_colores = explode(",", ($colores . ",")); $data_colores = array();
                                foreach ($array_colores as $value2) {  if (strlen($value2) != 0) {  $id_color = $this->db->query("SELECT * FROM colores WHERE UPPER(nombre) = UPPER('" . trim($value2) . "') AND borrado='no'"); if ($id_color->num_rows() != 0) { $id_c = $id_color->row(); $data_colores[] = array('producto_id' => $insert, 'color_id' => $id_c->id); } else { $this->db->trans_rollback(); echo '<link href="' . base_url() . 'assets/web/css/bootstrap.min.css" rel="stylesheet"><body><div class="modal-backdrop fade in" style="z-index:-1;"></div><div class="modalx fade"  role="dialog" style="display:block;opacity:1;"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><h3 class="modal-title"> Error al importar desde Excel</h3></div><div class="modal-body form" style="margin-bottom:15px;"> <label class="control-label col-md-3" style="width:100%;">El color ' . $value2 . ' de la celda ' . $i . ' no existe!</label></div><div class="modal-footer"><button type="button" onClick="history.back()" class="btn btn-danger" data-dismiss="modal">Atras</button></div></div></div></div></div></body>'; die;}} }
                                $this->obj_model->insert_multiple('producto_colores', $data_colores);
                            }

                            if (($insert)and ( strlen($medidas) != 0)) {
                                $array_medidas = explode(",", ($medidas . ","));  $data_medidas = array();
                                foreach ($array_medidas as $value3) { if (strlen($value3) != 0) {  $id_medida = $this->db->query("SELECT * FROM medidas WHERE UPPER(nombre) = UPPER('" . trim($value3) . "') AND borrado='no'"); if ($id_medida->num_rows() != 0) { $id_m = $id_medida->row(); $data_medidas[] = array('producto_id' => $insert, 'medida_id' => $id_m->id);  } else { $this->db->trans_rollback(); echo '<link href="' . base_url() . 'assets/web/css/bootstrap.min.css" rel="stylesheet"><body><div class="modal-backdrop fade in" style="z-index:-1;"></div><div class="modalx fade"  role="dialog" style="display:block;opacity:1;"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><h3 class="modal-title"> Error al importar desde Excel</h3></div><div class="modal-body form" style="margin-bottom:15px;"> <label class="control-label col-md-3" style="width:100%;">La medida ' . $value3 . ' de la celda ' . $i . ' no existe!</label></div><div class="modal-footer"><button type="button" onClick="history.back()" class="btn btn-danger" data-dismiss="modal">Atras</button></div></div></div></div></div></body>'; die;} }  }
