...
class Usuarios extends MY_Controller {
    ...
    public function index() {
        ...
        $this->data_view['planes'] = $this->generate_data_dropdown('planes', 'nombre', FALSE);
    ...
    public function ajax_add() {
        ...
        $this->form_validation->set_rules('roles_id', 'Rol', 'required|trim');
        $this->form_validation->set_rules('sistema_id', 'Sistema', 'required|trim');
        $this->form_validation->set_rules('plan_id', 'Plan', 'required|trim|numeric');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if($insert) mkdir("./uploads/".sha1($data['usuario']), 0777);
