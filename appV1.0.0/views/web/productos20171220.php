                                    ...
                                    <form class="form-inline">
                                        <div class="col-md-6 col-sm-6">
                                            <div class="products-number"> <strong>Mostrar</strong> 
                                                <a href="<?= base_url(); ?><?= str_replace(' ', '-', $categoria_actual) ?>/<?= str_replace(' ', '-', $subcategoria_actual) ?>/<?= (($sort == 'precio_mayorista')and ( $order == 'desc')) ? $sort . '_' . $order : $sort ?>/12/<?= 1 ?>" class="btn btn-default btn-sm <?= ($per_page == 12) ? 'btn-primary' : '' ?>">12</a> 
                                                <a href="<?= base_url(); ?><?= str_replace(' ', '-', $categoria_actual) ?>/<?= str_replace(' ', '-', $subcategoria_actual) ?>/<?= (($sort == 'precio_mayorista')and ( $order == 'desc')) ? $sort . '_' . $order : $sort ?>/24/<?= 1 ?>" class="btn btn-default btn-sm <?= ($per_page == 24) ? 'btn-primary' : '' ?>">24</a> 
                                                <a href="<?= base_url(); ?><?= str_replace(' ', '-', $categoria_actual) ?>/<?= str_replace(' ', '-', $subcategoria_actual) ?>/<?= (($sort == 'precio_mayorista')and ( $order == 'desc')) ? $sort . '_' . $order : $sort ?>/99999999/<?= 1 ?>" class="btn btn-default btn-sm <?= ($per_page == 99999999) ? 'btn-primary' : '' ?>">Todos</a> 
                                            </div>
                                        </div>
                                        <div class="col-md-6 col-sm-6">
                            ...
                            <?php if ($indice == 0): ?><div class="row products"><?php endif; ?>
                                ...
                                <div class="col-md-3 col-sm-4">
                                    ...
                                    <div class="product" id="producto_<?= $producto->id ?>">
                                        <div class="image" style="height: 180px;"> <a href="<?= str_replace(' ', '-', site_url(strtolower($producto->categoria) . '/' . str_replace(' ', '-', $producto->titulo) . '-' . $producto->articulo . '-' . $producto->id)) ?>"> <img src="<?= base_url() . 'uploads/' . $producto->file1; ?>" alt="<?= $producto->titulo ?> art.<?= $producto->articulo ?>" class="img-responsive image1"> </a>
                                            <div class="quick-view-button"> <a href="#" my-link="<?= str_replace(' ', '-', site_url('web/modal_detalle/' . strtolower($producto->categoria) . '/' . str_replace(' ', '-', $producto->titulo) . '-' . $producto->articulo . '-' . $producto->id)) ?>" data-toggle="modal" data-target="#product-quick-view-modal" class="btn btn-default btn-sm">Vista rápida</a> </div>
                                        </div>
                                        <!-- /.image -->
                                        <div class="text">
                                            <h3><a href="<?= str_replace(' ', '-', site_url(strtolower($producto->categoria) . '/' . str_replace(' ', '-', $producto->titulo) . '-' . $producto->articulo . '-' . $producto->id)) ?>"><?= $producto->titulo ?> art.<?= $producto->articulo ?></a></h3>
