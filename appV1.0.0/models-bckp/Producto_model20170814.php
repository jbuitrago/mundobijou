...
class producto_model extends Default_model {
    ...
    var $table = 'producto';
    var $column = array('producto.id', 'titulo', 'articulo', 'categorias.nombre AS categoria', 'subcategorias.nombre AS subcategoria', 'precio_mayorista', 'precio_revendedor', 'stock');
    ...
    public function get_ofertas() {
        ...
        $this->db->select('id,titulo,descripcion_corta,file1,precio_mayorista,articulo,slug');
    ...
    function get_productos($limit, $start, $sort, $order, $categoria, $subcategoria) {
        ...
        $this->db->select('*, (SELECT COUNT(*) FROM pedido_items WHERE tipo_producto="producto" AND producto_id=producto.id GROUP BY producto.id) AS moresell', FALSE);
