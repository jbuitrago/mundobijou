...
class Web extends CI_Controller {
    ...
    public function send()
    {
      ...
      //  $insert_log = $this->contacto->save($data);
            ...
            $this->email->to('mauro_rc2@hotmail.com');
            ...
            $this->email->message('<html><head></head><body>
                            <u>NOMBRE: </u>' . $this->input->post('firstname') . '<br>'
                . '<u>APELLIDO: </u>' . $this->input->post('lastname') . '<br>'
               // . '<u>EMPRESA: </u>' . $this->input->post('empresa') . '<br>'
                . '<u>EMAIL: </u>' . $this->input->post('email') . '<br>'
                . '<u>TELEFONO: </u>' . $this->input->post('te') . '<br>'
                . '<u>COMENTARIO: </u>' . nl2br($this->input->post('message')) . '</body></html>');
            $this->email->send();
var_dump($this->email->print_debugger());
            /* if (!$this->email->send(FALSE))
            {
                ...
                echo json_encode(array("status" => true, "errores" => validation_errors()));
