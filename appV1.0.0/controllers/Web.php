...
class Web extends MY_Controller {
    ...
    var $cantidad_combos = 0;
    var $metadescripciones;
    ...
    function __construct() {
        ...
        $this->load->model("Banners_internos_model", "banners_internos");
        ...
        $this->load->model("Subcategorias_model", "subcategoria");

        $this->load->model("Metadescripciones_config_model", "metadescripcion");
        ...
        $this->cantidad_combos = $this->combo->get_cantidad_combos();

        $this->set_emails();

        $this->set_metadescripciones();
    ...
    public function index() {
        ...
        $data['mostrar_categoria_combos'] = $this->cantidad_combos;

        $data['metadescripcion'] = $this->metadescripciones->metadescripcion_inicio;
    ...
    public function productos() {
        ...
        if (!empty($this->uri->segment(1)) && ($this->uri->segment(1) == 'productos' || $this->uri->segment(1) == 'combos')) {
        ...
        $categoria = (!empty($this->uri->segment(2 - $to_rest_segment))) ? $this->get_id_sub_or_categoria($this->uri->segment(2 - $to_rest_segment)) : 0;

        $sub_categoria = (!empty($this->uri->segment(3 - $to_rest_segment))) ? $this->get_id_sub_or_categoria($this->uri->segment(3 - $to_rest_segment)) : 0;

        $sort = (!empty($this->uri->segment(4 - $to_rest_segment))) ? $this->uri->segment(4 - $to_rest_segment) : 'precio_mayorista';

        if ($this->uri->segment(4 - $to_rest_segment) == 'precio_mayorista_desc') {
            $order = 'desc';
        ...
        if ($this->uri->segment(5 - $to_rest_segment) != 'nofilter') {
            $filtro = $this->uri->segment(5 - $to_rest_segment);
        }
        if(empty($filtro)){
            $filtro = 'nofilter';
        }

        $data_categoria = $this->categoria->get_by_id($categoria);

        if ($sub_categoria != 0) {
            $data_categoria = $this->subcategoria->get_by_id($sub_categoria);
        }

        $_SESSION['last_url_view'] = ((!empty($this->uri->segment(2 - $to_rest_segment))) ? ($this->uri->segment(2 - $to_rest_segment)) : '0') . '/';

        $_SESSION['last_url_view'] .= '/' . (!empty($this->uri->segment(3 - $to_rest_segment)) && ($this->uri->segment(3 - $to_rest_segment) != 'favicon.png')) ? ($this->uri->segment(3 - $to_rest_segment)) : '0';
        ...
        $data['metadescripcion'] = $data_categoria->metadescripcion;
        ...
        $data_index['filtro'] = $filtro;

        $data_index['banners_internos'] = $this->banners_internos->get_banners();

        //numero de pagina actual, por defecto es la 1
        $page_number = ($this->uri->segment(7 - $to_rest_segment)) ? $this->uri->segment(7 - $to_rest_segment) : 1;

        //preparo la configuracion para la paginacion.
        $config = $this->get_config_pagination($categoria, $sub_categoria, $to_rest_segment);

        $offset = ($page_number - 1) * $config['per_page'];

        $data_index['productos'] = $this->producto->get_productos($config['per_page'], $offset, $sort, $order, $categoria, $sub_categoria, $filtro);
        ...
        $data_index['my_page'] = $to_rest_segment;
    ...
    public function registro() {
        ...
        if (!$this->session->userdata('cliente_logged_in')) {
            ...
            if ($resultado_registrar['status']) {
                ...
                $data['metadescripcion'] = $this->metadescripciones->metadescripcion_registro;
    ...
    public function detalle($id = '') {
        ...
        //$_SESSION['last_url_view'] = ((!empty($this->uri->segment(2 - $to_rest_segment))) ? ($this->uri->segment(2 - $to_rest_segment)) : 0).'/'.(!empty($this->uri->segment(3 - $to_rest_segment))) ? ($this->uri->segment(3 - $to_rest_segment)) : 0;
    ...
    public function carro() {
        ...
        $data['metadescripcion'] = $this->metadescripciones->metadescripcion_carro;
    ...
    public function contacto() {
        ...
        $data['metadescripcion'] = $this->metadescripciones->metadescripcion_contacto;
    ...
    public function red_vendedores() {

        $data['opcion'] = "index";

        $data['title'] = "Red Vendedores";
        ...
        $data['metadescripcion'] = $this->metadescripciones->metadescripcion_red_vendedores;
        ...
        $data_final['section'] = $this->load->view('web/red_vendedores', $data_index, true);
    ...
    public function combos() {
        ...
        $categoria = (!empty($this->uri->segment(2 - $to_rest_segment))) ? $this->uri->segment(2 - $to_rest_segment) : 0;

        $sub_categoria = (!empty($this->uri->segment(3 - $to_rest_segment))) ? $this->uri->segment(3 - $to_rest_segment) : 0;
        ...
        $data['metadescripcion'] = $this->metadescripciones->metadescripcion_combos;
        ...
        $data_index['combos'] = $this->combo->get_combos($config['per_page'], $offset, $sort, $order, 0, 0, $filtro);
    ...
    public function terminos_y_condiciones() {
        ...
        $data['metadescripcion'] = $this->metadescripciones->metadescripcion_terminos_condiciones;
    ...
    public function cart_add() {
        ...
        $_SESSION['last_url_view'] = $this->producto->get_categoria_for_uri($this->input->post('id'));

        $_SESSION['last_url_view'] .= '/' . $this->producto->get_subcategoria_for_uri($this->input->post('id'));
    ...
    function check_user_email() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($insert) {
                ...
                $mail->Subject = "=?UTF-8?B?" . base64_encode("CAMBIO DE CONTRASEÑA") . "?=";
                ; // Este es el titulo del email.

                $mail->Body = utf8_encode($this->load->view('email_templates/olvide_pass', $datos, true)); // Mensaje a enviar
    ...
    private function get_config_pagination($categoria, $sub_categoria, $page = 'productos') {
        ...
        if ($page == 0 || $page == 'combos') {

            if ($page === 0) {
                //echo '<br>entro por cero: '.$page.'<br>';
                $page = 'productos';
            }
            ...
            $config["base_url"] = base_url() . $this->contruct_uri();
        }

        if ($page != 'combos') {
            //echo '<br>entro por != combos: '.$page.'<br>';
            $page = 'productos';
        }
        ...
        if ($page == 'productos') {
            $config["total_rows"] = $this->producto->record_count($categoria, $sub_categoria, $filtro);
        } else {
            $config["total_rows"] = $this->combo->record_count($categoria, $sub_categoria, $filtro);
        }

        $config["per_page"] = (!empty($this->uri->segment(6 - $to_rest_segment))) ? $this->uri->segment(6 - $to_rest_segment) : '12';

        $config['uri_segment'] = 7 - $to_rest_segment;
    ...
    private function contruct_uri() {
        ...
        $sort = (!empty($this->uri->segment(4 - $to_rest_segment))) ? $this->uri->segment(4 - $to_rest_segment) : 'titulo';

        $page = (!empty($this->uri->segment(7 - $to_rest_segment))) ? $this->uri->segment(7 - $to_rest_segment) : '1';

        $per_page = (!empty($this->uri->segment(6 - $to_rest_segment))) ? $this->uri->segment(6 - $to_rest_segment) : '12';

        $filtro = (!empty($this->uri->segment(5 - $to_rest_segment))) ? $this->uri->segment(5 - $to_rest_segment) : 'nofilter';

        //$extra = (!empty($_GET['search_ter']))? '/?search_ter='.$_GET['search_ter']: '';

        return '/' . $categoria . '/' . $sub_categoria . '/' . $sort . '/' . $filtro . '/' . $per_page;
    }
    ...
    public function send() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            //Estas dos líneas, cumplirían la función de encabezado (En mail() usado de esta forma: “From: Nombre <correo@dominio.com>”) de //correo.
            $mail->AddAddress($this->email_contacto); // Esta es la dirección a donde enviamos
            $mail->IsHTML(true); // El correo se envía como HTML
...
//También podríamos agregar simples verificaciones para saber si se envió:
    ...
    public function busqueda() {
        ...
        $data['metadescripcion'] = $this->metadescripciones->metadescripcion_busqueda;
        ...
        $data_index['productos'] = $this->producto->get_productos(1000000, $offset, $sort, $order, $categoria, $sub_categoria);

        $data_index['combos'] = $this->combo->get_combos(1000000, $offset, $sort, $order, 0, 0);
    ...
    public function get_id_sub_or_categoria($param) {

        $temporal_data = explode('-', $param);

        $cant = count($temporal_data);

        return $temporal_data[$cant - 1];
    }

    public function blogs() {

        $categoria = (!empty($this->uri->segment(1))) ? $this->uri->segment(1) : 0;

        $sub_categoria = (!empty($this->uri->segment(2))) ? $this->uri->segment(2) : 0;

        $sort = (!empty($this->uri->segment(3))) ? $this->uri->segment(3) : 'precio_mayorista';

        if ($this->uri->segment(3) == 'precio_mayorista_desc') {
            $order = 'desc';
            $sort = 'precio_mayorista';
        } else {
            $order = 'asc';
        ...
        echo '<br>subcategoria: ' . $sub_categoria;
    ...
    public function show_404($param = '') {
        ...
        $data_final['header'] = $this->load->view('web/header', $data['categorias'], true);

        $data_final['section'] = $this->load->view('web/error_page', $data, true);
    ...
    private function set_metadescripciones() {

        $this->metadescripciones = $this->metadescripcion->get_last();
    }
