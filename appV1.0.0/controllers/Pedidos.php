...
class pedidos extends MY_Controller {
    ...
    var $data_view = array();
    var $otra_accion = ''
            . '         <a class="btn btn-sm btn-default" href="javascript:void()" title="Generar comprobante" onclick="generar_pdf(#IDENTIFICADOR)"><i class="fa fa-print"></i></a>';
    ...
    private function actualizar_items($pedido_id) {
        ...
        foreach ($_POST['cart'] as $p) {
            ...
            if ($p['hay'] == 'no') {
                ...
                $this->enviar_alerta(array('titulo' => $p['name'], 'imagen' => $p["imagen"]));
            }
        }
    ...
    public function generate_pdf($id) {
        ...
        $mpdf->SetTitle("Pedido  - MundoBijou");

        $mpdf->SetAuthor("MundoBijou");
        ...
        $msg['localidades'] = $this->generate_data_dropdown('localidad', 'nombre', FALSE);
        ...
        $html = $this->load->view($this->folder_view . '/pdf', $msg, TRUE);
        
        $mpdf->WriteHTML($html);

        $mpdf->Output(date('YmdHis') . 'pedido-' . str_pad($pedido->id, 4, "0", STR_PAD_LEFT) . '.pdf', 'D');
