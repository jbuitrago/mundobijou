        ...
        if ($this->session->userdata('cliente_logged_in') && !empty($this->cart->contents()) && !empty($this->session->userdata('direccion_pedido')) && !empty($this->session->userdata('formapago_pedido'))) {
            ...
            if ($pedido_grabado['status']) {
                ...
                if (!empty($pedido_grabado['productos_sin_stock'])) {
                    $this->enviar_email(array('email_to' => 'negocifacundo@hotmail.com', 'subject' => 'Hay productos sin stock', 'items' => $pedido_grabado['productos_sin_stock']), 'quiebre');
                }
...
//También podríamos agregar simples verificaciones para saber si se envió:
    ...
    public function update_pedido() {
        ...
        foreach ($_POST['cart'] as $p) {
            ...
            if (!$this->pedido->hay_stockb($p['tipo_producto'], $p['id'])) {
        ...
        if (count($items_sin_stock) > 0) {
            $this->enviar_email(array('email_to' => 'negocifacundo@hotmail.com', 'subject' => 'Hay productos sin stock', 'items' => $items_sin_stock), 'quiebre');
        }
    }
